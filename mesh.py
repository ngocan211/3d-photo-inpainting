...
import torch
import os
from utils import open_small_mask, refine_depth_around_edge, read_MiDaS_depth
...
from mesh_tools import get_depth_from_maps, get_map_from_ccs, get_edge_from_nodes, get_rgb_from_nodes, \
    crop_maps_by_size, convert2tensor, recursive_add_edge, update_info, filter_edge, \
    depth_inpainting
...
def remove_redundant_edge(mesh, edge_mesh, edge_ccs, info_on_pix, config, redundant_number=1000, invalid=False,
                          spdb=False):
    point_to_amount = {}
    point_to_id = {}
...
def judge_dangle(mark, mesh, node):
    ...
    mesh_neighbors = [xx for xx in mesh_neighbors if
                      0 < xx[0] < mesh.graph['H'] - 1 and 0 < xx[1] < mesh.graph['W'] - 1]
...
def context_and_holes(mesh, edge_ccs, config, specific_edge_id, specific_edge_loc, depth_feat_model,
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        if inpaint_iter == 0:
            depth_dict = get_depth_from_maps(context_map, mask_map, context_depth, mesh.graph['H'], mesh.graph['W'],
            ...
            depth_dict = depth_inpainting(None, None, None, None, mesh, config, union_size, depth_feat_model, None,
    ...
    if inpaint_iter == 0:
        ...
        for ecnt_id, ecnt_cc in enumerate(extend_context_ccs):
            ...
            _, tmp_label_map = cv2.connectedComponents((init_invalid_context_map | tmp_context_map).astype(np.uint8),
                                                       connectivity=8)
...
def DL_inpaint_edge(mesh,
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(
        ...
        if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
            ...
            tensor_edge_dict['output'] = (depth_edge_output > config['ext_edge_threshold']).float() * tensor_edge_dict[
                'mask'] + tensor_edge_dict['edge']
        else:
                ...
                if config.get('repeat_inpaint_edge') is True:
                    for _ in range(2):
                        ...
                        depth_edge_output = depth_edge_model.forward_3P(tensor_edge_dict['mask'],
                                                                        ...
                                                                        cuda=device)
                        depth_edge_output = depth_edge_output.cpu()
                        depth_edge_output = (depth_edge_output > config['ext_edge_threshold']).float() * \
...
def write_ply(image,
    ...
    input_mesh, info_on_pix, edge_mesh, depth, aft_mark = remove_dangling(input_mesh, edge_ccs, edge_mesh, info_on_pix,
                                                                          image, depth, config)
    ...
    if config['extrapolate_border'] is True:
        ...
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth,
    ...
    edge_maps, extend_context_ccs, extend_edge_ccs, extend_erode_context_ccs = \
        context_and_holes(input_mesh, new_edge_ccs, config, specific_edge_id, specific_edge_loc, depth_feat_model,
    ...
    k_00 = input_mesh.graph['cam_param_pix_inv'][0, 0]
    k_02 = input_mesh.graph['cam_param_pix_inv'][0, 2]
    k_11 = input_mesh.graph['cam_param_pix_inv'][1, 1]
    k_12 = input_mesh.graph['cam_param_pix_inv'][1, 2]
    w_offset = input_mesh.graph['woffset']
    h_offset = input_mesh.graph['hoffset']

    logging.error('info_on_pix()')
    for pix_xy, pix_list in info_on_pix.items():
        for pix_idx, pix_info in enumerate(pix_list):
            ...
            sx = pix_xy[0]
            sy = pix_xy[1]
            abs_z = abs(pix_info['depth'] if pix_info.get('real_depth') is None else pix_info['real_depth'])
            str_pt = [
            ...
            if input_mesh.has_node((sx, sy, pix_info['depth'])) is False:
                return False
                continue

            if pix_info.get('overlap_number') is not None:
                str_color = [x for x in
                             (pix_info['color'] / pix_info['overlap_number']).astype(np.uint8).tolist()]
            else:
                str_color = [x for x in pix_info['color'].tolist()]
            ...
            if pix_info.get('edge_occlusion') is True:
                ...
                if pix_info.get('inpaint_id') is None:
                    ...
                    str_color.append((pix_info.get('inpaint_id') + 1))
    ...
    hFov = float(input_mesh.graph['hFov'])
    vFov = float(input_mesh.graph['vFov'])
...
def output_3d_photo_(image, sample, ply_data, config, id_type):
    ...
    _, __ = output_3d_photo(verts.copy(), colors.copy(), faces.copy(), copy.deepcopy(Height),
                            ...
                            config.get('original_w'), border=[int(xx) for xx in [top, down, left, right]], depth=depth,
                            normal_canvas=None, all_canvas=None,
                            mean_loc_depth=mean_loc_depth, id_type=id_type)
